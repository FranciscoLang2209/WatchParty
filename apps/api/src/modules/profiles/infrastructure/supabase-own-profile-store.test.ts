 ...
 * devuelve el mismo builder (registrado vía `vi.fn` para poder aserirse
 * sobre los argumentos) y es "thenable" para soportar tanto
 * `await client.from(...).select(...)` (sin terminal, como en `listTeams`)
 * como el uso con `.single()`/`.maybeSingle()`.
 */
...
describe('SupabaseOwnProfileStore', () => {
  ...
  describe('saveOwnProfile()', () => {
    ...
    it('rechaza un favoriteTeamId que no es un UUID válido, sin consultar la base', async () => {
      const client = makeFakeClient({});
      const store = new SupabaseOwnProfileStore(asSupabaseClient(client));
